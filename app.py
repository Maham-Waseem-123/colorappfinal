# ============================================
# STREAMLIT RESERVOIR ENGINEERING APP (THEMED WITH WHITE TEXT & GLASS SIDEBAR)
# ============================================
...
.css-1d391kg { background-color: rgba(255,255,255,0.15) !important; backdrop-filter: blur(10px); color: white !important; }
div.stButton > button { background-color: rgba(255,255,255,0.2); color:white; font-weight:bold; border-radius:8px; border:1px solid rgba(255,255,255,0.3);}
...
def train_model(df):
    ...
    gbr = GradientBoostingRegressor(loss="absolute_error", learning_rate=0.1,
                                    n_estimators=600, max_depth=1, random_state=42, max_features=5)
...
page = st.sidebar.radio("Select a Page:", ["Reservoir Engineering Dashboard",
...
def make_binned_lineplot(df, xcol, bins=10):
    ...
    fig = px.line(binned_df, x='bin_center', y='Production (MMcfge)',
    ...
    fig.update_layout(
        ...
        xaxis=dict(showgrid=False, title=dict(font=dict(color='white', size=14, family="Segoe UI", bold=True)),
                   tickfont=dict(color='white', size=12, family="Segoe UI")),
        yaxis=dict(showgrid=False, title=dict(font=dict(color='white', size=14, family="Segoe UI", bold=True)),
                   tickfont=dict(color='white', size=12, family="Segoe UI"))
    )

    fig.update_xaxes(range=[binned_df['bin_center'].min(), binned_df['bin_center'].max()],
...
if page == "Reservoir Engineering Dashboard":
    ...
    features_to_plot = ["Porosity","Additive per foot (bbls)","Water per foot (bbls)","Proppant per foot (lbs)"]
...
# ------------------- PAGE 3: ECONOMIC ANALYSIS -------------------
    ...
    df["CAPEX"] = (base_drilling_cost*df["Depth (feet)"] + base_completion_cost*df["Gross Perforated Interval (ft)"] +
    ...
    if "predicted_production" in st.session_state:
        ...
        new_capex = (base_drilling_cost*df["Depth (feet)"].mean() + base_completion_cost*df["Gross Perforated Interval (ft)"].mean() +
